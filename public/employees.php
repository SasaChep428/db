...
// Ограничение доступа
$allowedRoles = ['Administrator', 'Project Manager'];
restrictAccess($allowedRoles);

// Обработка действий администратора
if ($access_type === 'Administrator' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (isset($_POST['action']) && $_POST['action'] === 'change_password') {
            ...
            $stmt = $pdo->prepare("INSERT INTO audit_log (entity_type, entity_id, changed_field, old_value, new_value, changed_by, changed_at) 
                                   VALUES ('employee', ?, 'password', '***', '***', ?, CURRENT_TIMESTAMP)");
            $stmt->execute([$target_employee_id, $employee_id]);
            
            $_SESSION['message'] = "Пароль для сотрудника ID $target_employee_id успешно изменен.";
            $_SESSION['message_type'] = 'success';
        } elseif (isset($_POST['action']) && $_POST['action'] === 'toggle_status') {
            $target_employee_id = (int)($_POST['employee_id'] ?? 0);
            $stmt = $pdo->prepare("SELECT is_active FROM employees WHERE employee_id = ?");
            $stmt->execute([$target_employee_id]);
            $current_status = $stmt->fetchColumn();
            
            $new_status = $current_status ? 0 : 1;
            $stmt = $pdo->prepare("UPDATE employees SET is_active = ? WHERE employee_id = ?");
            $stmt->execute([$new_status, $target_employee_id]);
            
            // Логирование изменения статуса
            $stmt = $pdo->prepare("INSERT INTO audit_log (entity_type, entity_id, changed_field, old_value, new_value, changed_by, changed_at) 
                                   VALUES ('employee', ?, 'is_active', ?, ?, ?, CURRENT_TIMESTAMP)");
            $stmt->execute([$target_employee_id, $current_status, $new_status, $employee_id]);
            
            $_SESSION['message'] = "Статус сотрудника ID $target_employee_id изменен.";
            $_SESSION['message_type'] = 'success';
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <h2 class="mt-4">Управление сотрудниками</h2>
                    ...
                    <table class="table table-striped" id="employeesTable">
                        <thead>
                            <tr>
                                ...
                                <th>Значение</th>
                                <th>Первичный</th>
                                <?php if ($access_type === 'Administrator'): ?>
                                    <th>Действия</th>
                                <?php endif; ?>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (empty($employees)): ?>
                                <tr><td colspan="<?php echo $access_type === 'Administrator' ? 10 : 9; ?>" class="text-center">Нет сотрудников.</td></tr>
                            <?php else: ?>
                                <?php foreach ($employees as $employee): ?>
                                    <?php if (empty($employee['contacts'])): ?>
                                        <tr>
                                            ...
                                            <td><?php echo $employee['is_active'] ? 'Активен' : 'Неактивен'; ?></td>
                                            ...
                                            <?php if ($access_type === 'Administrator'): ?>
                                                <td>
                                                    ...
                                                    <button type="button" class="btn btn-sm btn-primary mb-1" data-bs-toggle="modal" 
                                                            data-bs-target="#changePasswordModal<?php echo htmlspecialchars($employee['employee_id']); ?>">
                                                        Сменить пароль
                                                    </button>
                                    ...
                                    <div class="modal fade" id="changePasswordModal<?php echo htmlspecialchars($employee['employee_id']); ?>" 
                                         tabindex="-1" aria-labelledby="changePasswordModalLabel<?php echo htmlspecialchars($employee['employee_id']); ?>" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="changePasswordModalLabel<?php echo htmlspecialchars($employee['employee_id']); ?>">
                                                        Смена пароля для <?php echo htmlspecialchars($employee['first_name'] . ' ' . $employee['last_name']); ?>
                                                    </h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <form method="POST">
                                                        ...
                                                        <div class="mb-3">
                                                            <label for="new_password<?php echo htmlspecialchars($employee['employee_id']); ?>" class="form-label">Новый пароль</label>
                                                            <input type="password" name="new_password" 
                                                                   id="new_password<?php echo htmlspecialchars($employee['employee_id']); ?>" 
    ...
    <script>
        $(document).ready(function () {
            $('#employeesTable').DataTable({
                ...
                columnDefs: [
                    <?php if ($access_type === 'Administrator'): ?>
                        { orderable: false, targets: 9 } // Отключение сортировки для столбца "Действия"
                    <?php endif; ?>
                ]
            });
        });
